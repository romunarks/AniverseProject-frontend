...
export const ListaDetailPage: React.FC = () => {
    ...
    }, [listaId, loadListaData]);

    const loadListaData = React.useCallback(async () => {
        setLoading(true);
        setError(null);

        try {
            // ✅ Cargar datos secuencialmente para evitar problemas de tipos
    ...
    if (loading) {
        return (
            <div className="container mx-auto px-4 py-16 flex justify-center">
                <div className="animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-aniverse-purple"></div>
            </div>
        );
    }
...
// Placeholders para los modales (los crearemos después)
const AddAnimeToListModal: React.FC<{
    isOpen: boolean;
    listaId: number;
    onClose: () => void;
    onSuccess: () => void;
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                <h2 className="text-xl font-bold mb-4">Agregar Anime a la Lista</h2>
                <p className="text-gray-600 mb-4">Modal para agregar anime - implementaremos después</p>
                <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 px-4 py-2 border rounded-lg">Cancelar</button>
                    <button onClick={onSuccess} className="flex-1 px-4 py-2 bg-aniverse-purple text-white rounded-lg">Agregar</button>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 w-full max-w-md">
                ...
                <p className="text-gray-600 mb-4">Modal para editar lista - implementaremos después</p>
                <div className="flex gap-3">
                    <button onClick={onClose} className="flex-1 px-4 py-2 border rounded-lg">Cancelar</button>
                    <button onClick={onSuccess} className="flex-1 px-4 py-2 bg-aniverse-purple text-white rounded-lg">Guardar</button>
                </div>
            </div>
        </div>
    );
