...
// CAMBIO PRINCIPAL: Usar ruta relativa cuando hay proxy
// En desarrollo, Vite proxy redirigirá /api a http://localhost:8080/api
    ...
    async (error: AxiosError) => {
        ...
        if (status === 401 && !originalRequest._retry) {
            ...
            try {
                ...
                // CORREGIDO: Usar la misma base URL para refresh
                const refreshUrl = import.meta.env.PROD
                    ? `${import.meta.env.VITE_API_URL || 'http://localhost:8080/api'}/refresh`
                    : '/api/refresh';
