...
export const estadisticaService = {
    ...
    // Agrega estos métodos al final de tu estadisticaService.ts, antes del cierre del objeto:

    /**
     * Obtener animes más recientes
     */
    getMostRecentAnimes: async (limit: number = 10): Promise<Anime[]> => {
        try {
            const response = await api.get<AniverseResponse<Anime[]>>(`/estadisticas/most-recent?limit=${limit}`);
            if (response.data?.success) {
                return response.data.data || [];
            }
            // Fallback
            const fallback = await api.get(`/animes?page=0&size=${limit}&sortBy=createdAt&direction=desc`);
            return fallback.data?.data?.content || [];
        } catch (error) {
            console.error('Error getMostRecentAnimes:', error);
            return [];
        }
    },

    /**
     * Obtener animes más votados
     */
    getMostVotedAnimes: async (limit: number = 10): Promise<Anime[]> => {
        try {
            const response = await api.get<AniverseResponse<Anime[]>>(`/estadisticas/most-voted?limit=${limit}`);
            if (response.data?.success) {
                return response.data.data || [];
            }
            // Fallback
            const fallback = await api.get(`/animes?page=0&size=${limit}&sortBy=id&direction=desc`);
            return fallback.data?.data?.content || [];
        } catch (error) {
            console.error('Error getMostVotedAnimes:', error);
            return [];
        }
    },
